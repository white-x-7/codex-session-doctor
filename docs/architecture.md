...
rollout 用"同目录临时文件 + 改名"的方式落地，不原地截断重写。原地写一旦在写一半时被打断（Ctrl-C、断电、系统回收），用户的历史文件会留下残缺版本；改名在同一文件系统内是原子的，结果要么是旧内容，要么是完整的新内容。临时文件会继承原文件权限，成功后清理干净。
...
读不了的 rollout（权限不足、被占用、意外是个目录）必须作为警告报出来。静默跳过会让用户以为"没有需要修复的会话"，而实际上只是没读到。
